...
export default function App() {
    ...
    return <DeviceFrame userName={user.nickname} onSwitchUser={switchUser} onSwitchGuest={switchGuest} onResetDeals={resetDeals} version={version} onSwitchVersion={switchVersion}>
        ...
        {salesManagement && <SalesManagementScreen key={dealsRevision} posts={versionPosts} products={managedProducts} ownerId={user.id} productVersion={version} onBack={() => { window.location.hash = accountHash(routePrefix, "my"); }} onOpenHistory={openHistory} onOpenListing={key => { openedFromHome.current = false; window.location.hash = hashForKey(key); }} onDelete={isPublicDemo ? undefined : removePost} />}
